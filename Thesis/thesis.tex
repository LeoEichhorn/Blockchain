...
The core part of the simulator represents an abstracted framework for blockchain architectures. Compared to the example blockchain protocol presented by Bitcoin, a few adaptations have been made 
\begin{itemize}
\item nodes send their entire version of the blockchain
\item blockchain represented by its length
\item not transactions or blocks
\item nodes start mining on received blockchain if it is longer than the blockchain currently being mined on
\item generating random numbers instead of increment nonce + hash
\end{itemize}
\subsection{Classes}
\begin{figure}[ht]
	\centering
...
\subsubsection{Node}
The node represents a basic implementation of a honest miner adhering to the blockchain protocol. Each node contains a reference to every other node in the network using the peer class. Once a new block is mined, it is added to the local blockchain and, after the corresponding latency, a copy of the blockchain is sent to all peers. The node will only start mining on a newly received blockchain, if it is longer than the local blockchain copy. To simplify execution of multiple simulation runs, each node can be reset to their initial state. To allow simulation of nodes diverging from the standard protocol, the class contains three abstract methods which can be implemented to alter the behavior of the node. 
...
The network acts as a central controller of the blockchain framework. It maintains references to all nodes in the network and provides simple routines for starting and stopping the mining process of all nodes in a synchronized manner. Once the network is started, the calling thread of \textit{run()} is blocked, until the execution of all nodes is completed. The nodes terminate their mining threads after the network is stopped, which is ideally done as a consequence of some end condition reached by the simulation.
...
Despite blockchain only represented by number of blocks not actual blocks,no distinction between stale main orphaned blocks, effect of stale blocks and branches still exist
